...
struct AddPersonSheet: View {
    ...
    private var notFoundCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(alignment: .top, spacing: 12) {
                ...
                Text("No users found with this username, invite them to PayUp or create a dummy user")
        ...
        VStack(alignment: .leading, spacing: 18) {
            VStack(alignment: .leading, spacing: 6) {
                Text("Create a dummy friend")
                    ...
                    .foregroundStyle(Color.appText)
                Text("and you can link them with your friend later when they join PayUp")
                    .font(.subheadline)
                    .foregroundStyle(Color.appText.opacity(0.58))
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private var shareURL: URL {
        URL(string: "https://payup.app/join")!
    }

    private var shareMessage: String {
        "Join me on PayUp so we can split expenses together!"
    }
...
struct InfiniteEmojiCarouselView: UIViewRepresentable {
    let baseEmojis: [String]
    @Binding var selectedEmoji: String
